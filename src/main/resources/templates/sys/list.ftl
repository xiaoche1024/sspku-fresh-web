...
<div class="box box-default">
    ...
    <!-- /.box-header -->
    <div class="box-body">
        <button type="button" class="btn btn-success" onclick="sysUser.addOrEdit()">新增</button>
    ...
    var sysUser = new function(){
        this.getDataList = function () {
            _table = $("#tb").dataTable({
                ...
                "bDestroy":true,
                ajax: {
                    url:contextPath + "/sysUser/listData",
                    method:'POST',
                    data: function (data) {
//                    data.status = $("#status").val();
                ...
                "columnDefs" : [{
                    "render" : function(data, type, row) {
                        var oper = '<a href="javascript:void(0)" onclick="sysUser.addOrEdit(\''+ row.userId +'\')">修改</a> | ';
                        oper += ('<a href="javascript:void(0)" onclick="sysUser.del(\''+ row.userId +'\')">删除</a>');
                        return oper;
                    },
                    "targets" : 3
                }],
        ...
        this.addOrEdit = function (id) {
            var url = contextPath + '/sysUser/editPage';
            if(id){
                url += "?id=" + id;
            }
